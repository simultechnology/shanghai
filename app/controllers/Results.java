...
public class Results extends Controller {
    ...
    public static Result complete() {
        ...
        final long result_id = Long.parseLong(data.get("result_id"));
        final int score = Integer.parseInt(data.get("score"));
        ...
        try {
            Ebean.execute(new TxRunnable() {
                ...
                public void run() {
                    ...
                    Map<String, Map> subjectCodeMap = makeScoreData(data);
            ...
            postProc(result_id);
            return ok(results.render(false));
        }
        postProc(result_id);
        return ok(results.render(true));
    }

    private static void postProc(long result_id) {
    ...
    private static Map<String, Map> makeScoreData(Map<String, String> data) {
        Map<String, Map> subjectScoreMap = new HashMap<String, Map>();
        for (int i = 1; i <= 20; i++) {
            ...
            Map<String, Integer> map = subjectScoreMap.get(subject_code);
            if (map == null) {
                map = new HashMap();
            ...
            subjectScoreMap.put(subject_code, map);
        }
